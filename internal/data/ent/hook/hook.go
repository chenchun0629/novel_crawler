...
// The NovelClueFunc type is an adapter to allow the use of ordinary
// function as NovelClue mutator.
type NovelClueFunc func(context.Context, *ent.NovelClueMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f NovelClueFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.NovelClueMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.NovelClueMutation", m)
	}
	return f(ctx, mv)
}
