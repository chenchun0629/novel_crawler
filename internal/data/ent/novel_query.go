...
// QueryClue chains the current query on the "clue" edge.
func (nq *NovelQuery) QueryClue() *NovelClueQuery {
	query := &NovelClueQuery{config: nq.config}
...
// WithClue tells the query-builder to eager-load the nodes that are connected to
// the "clue" edge. The optional arguments are used to configure the query builder of the edge.
func (nq *NovelQuery) WithClue(opts ...func(*NovelClueQuery)) *NovelQuery {
	query := &NovelClueQuery{config: nq.config}
...
func (nq *NovelQuery) sqlAll(ctx context.Context) ([]*Novel, error) {
	...
	if query := nq.withClue; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Novel)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Clue = []*NovelClue{}
		}
		query.withFKs = true
		query.Where(predicate.NovelClue(func(s *sql.Selector) {
			s.Where(sql.InValues(novel.ClueColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.novel_clue
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "novel_clue" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "novel_clue" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Clue = append(node.Edges.Clue, n)
		}
	}
